 ...
 * 
 */
package org.mule.wrapper.hbase;
 ...
 */
/*
 * This class same class as Hbase client Result just a workaround for Mule Devkit Generate Classes bug
 */
@InterfaceAudience.Public
@InterfaceStability.Stable
public class ResultWrapper implements CellScannable, CellScanner {
  private Cell[] cells;
  ...
  public static final ResultWrapper EMPTY_RESULT = new ResultWrapper();
   ...
   */
  @Deprecated
  public ResultWrapper(KeyValue [] cells) {
    this.cells = cells;
  }
   ...
   */
  @Deprecated
  public ResultWrapper(List<KeyValue> kvs) {
   ...
   */
  public static ResultWrapper create(List<Cell> cells) {
    return new ResultWrapper(cells.toArray(new Cell[cells.size()]), null);
  }

  public static ResultWrapper create(List<Cell> cells, Boolean exists) {
    if (exists != null){
      return new ResultWrapper(null, exists);
    }
    return new ResultWrapper(cells.toArray(new Cell[cells.size()]), null);
  }

  /**
   * Instantiate a ResultWrapper with the specified array of KeyValues.
   * <br><strong>Note:</strong> You must ensure that the keyvalues are already sorted.
   * @param cells array of cells
   */
  public static ResultWrapper create(Cell[] cells) {
    return new ResultWrapper(cells, null);
  }

  /** Private ctor. Use {@link #create(Cell[])}. */
  private ResultWrapper(Cell[] cells, Boolean exists) {
    this.cells = cells;
    this.exists = exists;
  }
   ...
   * the row from which this ResultWrapper was created.
   * @return row
   */
  public byte [] getRow() {
    if (this.row == null) {
   ...
   */
  public static void compareResults(ResultWrapper res1, ResultWrapper res2)
      throws Exception {
    if (res2 == null) {
   ...
   * @param other
   */
  public void copyFrom(ResultWrapper other) {
    this.row = null;
    this.familyMap = null;
